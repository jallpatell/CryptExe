      ...
      <main className="flex-grow pt-28 px-4 pb-10">
        <div className="flex flex-col md:flex-row gap-8 max-w-6xl mx-auto">
          ...
          <div className="md:w-1/2 w-full flex flex-col items-center md:items-start">
            ...
            {walletLoading ? (
              <div className="w-full max-w-md mt-10 space-y-4 animate-pulse">
                <div className="h-32 bg-gray-800/50 rounded-xl border border-gray-700"></div>
                <div className="h-10 w-40 bg-gray-800/50 rounded-md border border-gray-700"></div>
              </div>
            ) : (
              <>
                {(mnemonic === null || mnemonic.length === 0) && (
                  <button
                    className="mt-8 px-4 py-2 bg-white text-black rounded-lg hover:bg-black hover:text-white hover:border-2 hover:border-white transition-all"
                    onClick={displayMn}
                  >
                    Create Seed Phrase
                  </button>
                  ...
                  <>
                    <div className="max-w-md mt-10 bg-gray-900/50 backdrop-blur-md p-6 rounded-xl border border-gray-700 shadow-lg w-full">
                      <div className="flex justify-between items-center mb-4">
                        <h3 className="text-lg font-light font-mono">Seed Phrase</h3>
                        <button 
                      ...
                      <div className="grid grid-cols-3 gap-3 text-sm">
                        {mnemonic.map((word, index) => (
                          <div key={index} className="px-2 py-1 bg-gray-800/50 rounded-md text-center font-mono  border border-gray-700">
          ...
          <div className="md:w-1/2 w-full flex flex-col">
            {walletLoading ? (
              <div className="w-full mt-2 md:mt-0 animate-pulse space-y-4">
                <div className="h-10 w-full max-w-xs bg-gray-800/50 rounded-md border border-gray-700"></div>
                <div className="h-10 w-40 bg-gray-800/50 rounded-md border border-gray-700"></div>
                <div className="mt-8 h-64 bg-gray-800/50 rounded-xl border border-gray-700"></div>
              </div>
            ) : (
              <>
                {mnemonic !== null && mnemonic.length > 0 && (
                  <>
                    <div className="flex flex-row items-center gap-4 mt-2 md:mt-0">
                      <select
                        ...
                        value={selectedChain}
                        className="px-4 py-2 bg-gray-800/50 backdrop-blur-sm border border-gray-700 text-white rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 w-full max-w-xs"
                      >
                      ...
                      <button
                        className="px-4 py-2 bg-blue-500 text-white rounded-md hover:bg-black hover:text-white transition-all whitespace-nowrap"
                        onClick={async () => {
