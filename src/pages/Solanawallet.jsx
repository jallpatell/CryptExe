...
export default function SolanaWallet({ 
  ...
  const [expandedWalletIndex, setExpandedWalletIndex] = useState(null);
  ...
  return (
    <div className="max-w-2xl mx-auto"> {/* same as ETH wallet */}
      <div className="mt-8 space-y-4">
        {publicKeys.map((publicKey, index) => {
          const isExpanded = expandedWalletIndex === index;
          const currentActiveSubSection = isExpanded ? activeSubSection : null;
                ...
                <div className="mt-4 border-t border-gray-700 pt-4 space-y-4">
                  ...
                  {currentActiveSubSection === 'receive' && (
                    <div className="text-center bg-white p-4 rounded-md">
                      <p className="text-sm text-black font-mono font-extrabold mb-2">Scan QR to receive SOL</p>
                  ...
                  {currentActiveSubSection === 'send' && (
                    <div className="space-y-3 p-4 bg-gray-700/50 rounded-md">
                      ...
                      <div>
                        <label htmlFor={`amount-${index}`} className="block text-sm font-mono font-medium text-gray-400">Amount (SOL)</label>
                        <input
                          type="number"
                          id={`amount-${index}`}
                          className="mt-1 block w-full px-3 py-2 bg-gray-600 border border-gray-500 font-mono rounded-md shadow-sm placeholder-gray-400 text-white focus:outline-none focus:ring-blue-500 focus:border-blue-500"
                          value={sendAmount}
